      ...
      <tbody class="text-left accepted">
         ...
         while ($rows = $select_accepted_list->fetch(PDO::FETCH_OBJ)) {
            ...
            <tr class="<?php echo $status_class ?>" data-id="<?php echo $rows->id ?>">
               ...
               <?php if ($position == 'admin') { ?>
                  <td class="align-middle"><button type="button" name="email_button" class="btn btn-primary email_button email_single" data-email="<?php echo $rows->email ?>" <?php if ($rows->mail_sent == 1) echo 'disabled="true"' ?> data-name="<?php echo $rows->name ?>" data-action="email_single">
